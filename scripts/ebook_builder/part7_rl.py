# Part VII: Reinforcement Learning & Care Pathway Optimization (Chapters 31 - 36)
...
### 32.1 MDP Formalization: $\langle \mathcal{S}, \mathcal{A}, \mathcal{P}, \mathcal{R}, \gamma \rangle$
...
Where $r_t(\theta) = \frac{\pi_\theta(a_t \mid \mathbf{s}_t)}{\pi_{\theta_{\text{old}}}(a_t \mid \mathbf{s}_t)}$ is the probability ratio, and $\hat{A}_t$ is the Generalized Advantage Estimator (GAE).
