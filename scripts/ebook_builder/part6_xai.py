# Part VI: Explainable AI (XAI) (Chapters 27 - 30)
...
$$\mathbf{x}^* = \arg\min_{\mathbf{x}'} \mathcal{D}(\mathbf{x}, \mathbf{x}') \quad \text{subject to } f(\mathbf{x}') \le 0.30 \text{ and } \mathbf{x}' \in \text{Feasible}(\mathbf{x})$$
